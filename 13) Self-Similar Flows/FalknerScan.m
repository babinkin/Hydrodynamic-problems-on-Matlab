function [iter, eta, f, p] = FalknerScan(beta)
...
while (error < 0 || error > epsilon)
    ...
    while (eta(i) <= eta_max)
        ...
        delta1_f = h * p(i);
        delta1_p = h * q(i);
        delta1_q = - 1 * h * f(i) * q(i) - h * beta * (1 - p(i) ^ 2);
        
        delta2_f = h * (p(i) + delta1_p / 2);
        delta2_p = h * (q(i) + delta1_q / 2);
        delta2_q = - 1 * h * (f(i) + delta1_f / 2) * (q(i) + delta1_q / 2) - h * beta * (1 - (p(i) + delta1_p / 2) ^ 2);
        
        delta3_f = h * (p(i) + delta2_p / 2);
        delta3_p = h * (q(i) + delta2_q / 2);
        delta3_q = - 1 * h * (f(i) + delta2_f / 2) * (q(i) + delta2_q / 2) - h * beta * (1 - (p(i) + delta2_p / 2) ^ 2);
        
        delta4_f = h * (p(i) + delta3_p);
        delta4_p = h * (q(i) + delta3_q);
        delta4_q = - 1 * h * (f(i) + delta3_f) * (q(i) + delta3_q) - h * beta * (1 - (p(i) + delta3_p) ^ 2);
    ...
    if(error * preerror < 0 || abs(preerror) - abs(error) < 0)
